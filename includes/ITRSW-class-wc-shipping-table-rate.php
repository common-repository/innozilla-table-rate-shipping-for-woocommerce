...
*/
class ITRSW_FREE_WC_Shipping_Table_Rate extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->rates_table     = $wpdb->prefix . 'innozilla_woocommerce_shipping_table_rates';
	...
	public function init_form_fields() {
		...
		$this->instance_form_fields = array(
			'general_settings' => array(
				'title'       => __( 'General Settings', 'woocommerce-table-rate-shipping' ),
				'type'        => 'title',
				'description' => __( 'Define global shipping rules.', 'woocommerce-table-rate-shipping'),
				'default'     => ''
			),
			'title' => array(
			...
			'prices_include_tax' => array(
				...
				'options'     => array(
					'yes' => __( 'Yes, I will enter costs above inclusive of tax', 'woocommerce-table-rate-shipping' ),
					'no'  => __( 'No, I will enter costs above exclusive of tax', 'woocommerce-table-rate-shipping' ),
				),
			),
			'rates' => array(
				'title'       => __( 'Rates', 'woocommerce-table-rate-shipping' ),
				'type'        => 'title',
				'description' => __( 'Define table rates which are applied to an order.', 'woocommerce-table-rate-shipping'),
				'default'     => ''
			),
	...
	public function instance_options() {
		?>
		<div class="IFRSW_theme">
		<table class="form-table first-table">
			<?php
			$this->generate_settings_html( $this->get_instance_form_fields() );
			?>
			<tr>
				<th><?php _e( 'Table Rates', 'woocommerce-table-rate-shipping' ); ?></th>
				<td class="IFRSW-table-wrap mobile-restrict">
					<?php ITRSW_FREE_wc_table_rate_admin_shipping_rows( $this ); ?>
				</td>
				<td class="desktop-restrict">
					<h4>CAUTION: This Feature only works on Desktop. Please tilt the screen to landscape.</h4>
				</td>
			</tr>
			<?php if ( sizeof( WC()->shipping->get_shipping_classes() ) ) : ?>
				<tr valign="top" id="shipping_class_priorities">
					...
					<td class="forminp" id="shipping_rates">
						<?php ITRSW_FREE_wc_table_rate_admin_shipping_class_priorities( $this->instance_id ); ?>
					</td>
				</tr>
			<?php endif; ?>
		</table>
	...
	public function process_admin_options() {
		...
		ITRSW_FREE_wc_table_rate_admin_shipping_rows_process( $this->instance_id );
	}
	...
	private function save_abort_message( $message ) {
		$abort = WC()->session->get( ITRSW_FREE_WC_Table_Rate_Shipping::$abort_key );
		if ( empty( $abort ) ) {
			$abort = array();
		}

		$abort[ $this->instance_id ] = $message;
		WC()->session->set( ITRSW_FREE_WC_Table_Rate_Shipping::$abort_key, $abort );
	}
