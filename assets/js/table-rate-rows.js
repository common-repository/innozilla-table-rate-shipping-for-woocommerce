	...
	var wc_table_rate_rows = {
		init: function() {
			...
			$rates_table
				...
				.on( 'click', 'a.dupe', this.onDupeRate );onRemoveRate
		...
		onAddRate: function( event ) {
			...
			var numItems = $('#mainform').find('.table_rates tr').length;
			if (numItems == 0) {
				...
				$(this).parents().find('.pro-validation').slideDown();
			}
		},
		onRemoveRate: function( event ) {
			...
			if ( confirm( data.i18n.delete_rates ) ) {
				...
				$rates.find( 'tr td.check-column input:checked' ).each( function( i, el ) {
					...
					$(el).parents().find('.pro-validation').hide();
				});
